...
function Dialer() {
  ...
  const isCallBusy = isPlacingCall || isActive;
  const dialCountries = [
  ...
  const handleCall = async () => {
    ...
    try {
      ...
      const result = await startCall(destination);
      ...
      if (!result.success) {
        ...
        setSuccess(`Calling ${destination}...`);
        await fetchData(isMountedRef);
        setTimeout(() => {
            ...
            <div className="px-4 sm:px-6 py-3 sm:py-4 border-b border-gray-200 dark:border-slate-700 flex-shrink-0">
              <div className="flex items-center justify-center gap-2">
                ...
                <input
                  ...
                  disabled={isCallBusy || userNumbers.length === 0 || !subscriptionActive}
                />
              </div>
            </div>
