...
/**
 * IncomingCallNotification - Fallback for incoming calls
 * 
 * This shows as a backup if CallWindow/GlobalCallOverlay don't show the incoming call.
 * It will only show if:
 * - There's an incoming call
 * - AND we're not on Recents page (where CallWindow should show in dialer)
 */
export default function IncomingCallNotification() {
  ...
  // Only show if there's an incoming call
  const shouldShow = hasIncomingCall || callState === CALL_STATES.INCOMING;

  useEffect(() => {
    if (shouldShow) {
      setIsVisible(true);
      console.log('📞 IncomingCallNotification: Showing for:', remoteNumber);
      ...
      if ('vibrate' in navigator) {
        ...
        const vibrateInterval = setInterval(() => {
          if (shouldShow && 'vibrate' in navigator) {
            navigator.vibrate(vibratePattern);
          } else {
  ...
  }, [shouldShow, remoteNumber]);

  // Don't render if no incoming call
  if (!shouldShow || !isVisible) return null;

  // On Recents page, don't show this (CallWindow should show in dialer panel)
  // But show it as a fallback if CallWindow isn't working
  const isOnRecents = location === '/recents';
  
  // Show on mobile as full screen, or on desktop as notification
  const isMobile = window.innerWidth < 1024;

  if (isMobile) {
    // Mobile: Full screen incoming call UI
    return (
      <div className="fixed inset-0 z-[9999] bg-gradient-to-b from-slate-800 via-slate-900 to-black flex flex-col">
  ...
  // Desktop: Show notification banner (only if not on Recents, or as fallback)
  return (
