...
router.get("/admin/dashboard", authenticateUser, requireAdmin, async (req, res) => {
  try {
    ...
    const internalRealtimeFallback = Number(
      internalData?.realtime?.summary?.activeNow ??
      internalData?.realtime?.summary?.totalUsers ??
      0
    );
    const normalizedRealtimeActiveUsers = hasGaRealtimeUsers
      ? gaRealtimeActiveUsers
      : Number.isFinite(Number(selectedOverview?.realtimeActiveUsers))
        ? Number(selectedOverview.realtimeActiveUsers)
        : internalRealtimeFallback;
    ...
    selectedData = {
      ...
      overview: {
        ...
        realtimeActiveUsers: normalizedRealtimeActiveUsers
      },
      funnel: {
        ...selectedFunnel,
