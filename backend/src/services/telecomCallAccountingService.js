...
import { TELECOM_PRICING, resolveCountryMultiplier } from "../config/telecomPricingConfig.js";
...
export async function finalizeTelecomCallAccounting(callId, context = {}) {
  ...
  const attemptCredits = refreshed.attemptChargedAt || refreshed.attemptCharged ? 1 : 0;
  const durationCredits = Math.max(0, Number(refreshed.durationCreditsCharged || 0));
  const totalCreditsCharged = attemptCredits + durationCredits;
  const multiplier = resolveCountryMultiplier(destinationForCall(refreshed));
  const finalCharge = Math.ceil(totalCreditsCharged * multiplier);
